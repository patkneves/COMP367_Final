...
package com.assignment4.Gateway;
...
import com.assignment4.Gateway.model.HttpErrorBody;
...
public class GlobalExceptionHandler extends ResponseStatusExceptionHandler {
    ...
    public Mono<ResponseEntity<HttpErrorBody>> handleResponseStatusException(ResponseStatusException ex) {
        return Mono.just(ResponseEntity.status(ex.getStatusCode()).body(new HttpErrorBody(ex.getReason())));
    }
}
